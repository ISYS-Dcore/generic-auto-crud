...
## Cache
This library provides a simple way to use cache with your entities,
and it is designed to work with any entity that extends **GenericEntity** class.

## ⚠️ Cache Usage Warning
The caching mechanism provided in this project is designed for fast, in-memory lookups.
While it can greatly improve performance, **improper use of caching may lead to:**

- **High Memory Consumption** – All cached entities are kept in memory. Large datasets can quickly exceed available heap space.
- **Server Resource Pressure** – Reloading large caches can cause CPU and GC overhead, impacting system responsiveness.
- **Out-of-Sync Data** – Cached data may become stale if not properly refreshed or synchronized across distributed instances.

### ✅ Best Practices
- Only cache entities that are frequently accessed and relatively small in size.
- Monitor JVM memory (heap, GC) when enabling caching in production.
- Consider distributed cache invalidation (e.g., via message broker) if running multiple service instances.
- Always measure before enabling cache at scale.

# Cache Usage
In this section we will show how to use cache with this lib,
to do this you need to follow the steps below:

### Step 1 :
To use cache with this lib, you need to create a class that extends **GenericCache** class,
and then you can use the methods provided by this class to add secondary indexes and query the cache
````Java
GenericCache<String, MyEntity, MyService> cache = new GenericCache<>(myService);
````

### Step 2 :
You can add secondary indexes to the cache, this will allow you to query the cache in O(1) time complexity,
you can add multiple indexes to the cache, for example:
````Java
